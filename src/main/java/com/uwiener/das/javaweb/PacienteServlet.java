...
"                            Regresar</button></a>\n";
                        while(rs.next()){
                            sTabla+="<tr>\n" +
                                ...
                                "   <td><a href=\"./editarpaciente.jsp?idusuario="+rs.getInt(1)+"&nombre="+rs.getString(2)+"&apellido="+rs.getString(3)+"&dni="+rs.getInt(4)+"&correo="+rs.getString(5)+"&password"+rs.getInt(6)+"&direccion"+rs.getString(7)+"\"><button class=\"btn btn-warning\"> Editar</button></a></td>\n" +
                                "   <td><a href=\"./borrarpaciente.jsp?idusuario="+rs.getInt(1)+"&nombre="+rs.getString(2)+"&apellido="+rs.getString(3)+"&dni="+rs.getInt(4)+"&correo="+rs.getString(5)+"&password"+rs.getInt(6)+"&direccion"+rs.getString(7)+"\"><button class=\"btn btn-danger\"> Borrar</button></a></td>\n" +
                                "</tr>\n";
                        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            }else if(request.getParameter("accion").equals("E"))
            {
                ...
                st = cnx.prepareStatement("delete from tbpaciente where ID=?");                
                st.setString(1,request.getParameter("id").toString());
                st.executeUpdate();
                response.sendRedirect("./Paciente");
            }
